...
| F | Free text to ontology matching with a classifier | `06-ontology-matching.md` | Mapping records and spans to ontology concepts (medical, product, skills/occupation, clause, alarm, misconception taxonomies) with Choice/Score/Noul; candidate generation in DuckDB (FTS, VSS) then classifier verification; hierarchical narrowing under the 255-option cap; multi-label; the "other" bucket as a source of new concepts; entity-linking and ontology-matching prior art and benchmarks; how it plugs into the discovery loop. |
| S | Synthesis and MVP spec (after A–F) | `07-synthesis-and-mvp.md` | Consolidated architecture; decisions taken; repo layout; language choice; milestones with LoC; hello-world demo; test and CI plan using a free local backend; the constraints checklist merged across A–F. |
